...
output "curl_examples" {
  ...
  value = {
    create = "curl -X POST ${aws_api_gateway_stage.crud_api_stage.invoke_url}/items -H 'Content-Type: application/json' -d '{\"tableName\":\"${aws_dynamodb_table.crud_table.name}\",\"item\":{\"name\":\"Test Item\",\"description\":\"This is a test item\"}}'"
    read_all = "curl -X GET '${aws_api_gateway_stage.crud_api_stage.invoke_url}/items?tableName=${aws_dynamodb_table.crud_table.name}&operation=scan'"
    read_one = "curl -X GET '${aws_api_gateway_stage.crud_api_stage.invoke_url}/items/ITEM_ID?tableName=${aws_dynamodb_table.crud_table.name}'"
    update = "curl -X PUT ${aws_api_gateway_stage.crud_api_stage.invoke_url}/items/ITEM_ID -H 'Content-Type: application/json' -d '{\"tableName\":\"${aws_dynamodb_table.crud_table.name}\",\"id\":\"ITEM_ID\",\"item\":{\"name\":\"Updated Item\"}}'"
    delete = "curl -X DELETE '${aws_api_gateway_stage.crud_api_stage.invoke_url}/items/ITEM_ID?tableName=${aws_dynamodb_table.crud_table.name}&id=ITEM_ID'"
  }
}
